...
export default function AccountModal({ isOpen, account, onClose, onSuccess, viewOnly = false }) {
    ...
    useEffect(() => {
        const fetchOrderStatistics = async () => {
            if (!isOpen || !account || !internalViewOnly || account.role === 'admin') {
                setOrderStatistics({
                    totalOrders: 0,
                    totalSpent: 0,
                    activeOrders: 0
    ...
    }, [isOpen, account, internalViewOnly]);
                ...
                <div className={`bg-white rounded-2xl shadow-2xl border-2 w-full transform transition-all duration-300 ${internalViewOnly ? 'max-w-2xl' : 'max-w-md'} max-h-[90vh] flex flex-col`} style={{ borderColor: '#A86523' }}>
                    ...
                    <div className="flex items-center justify-between p-3 sm:p-4 lg:p-5 border-b shrink-0" style={{ borderColor: '#A86523' }}>
                        ...
                        <div className="flex items-center gap-2">
                            {internalViewOnly && !account.is_deleted && (
                                <button
                                    onClick={() => {
                    ...
                    <div className={`flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 ${internalViewOnly ? 'hide-scrollbar' : ''}`} style={internalViewOnly ? { scrollbarWidth: 'none', msOverflowStyle: 'none' } : {}}>
                        ...
                        {internalViewOnly ? (
                            ...
                            <div className="space-y-4">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 sm:gap-2">
                                                <div>
                                                    ...
                                                    <p className="text-xs sm:text-sm font-medium text-gray-900 truncate">{account.name || 'N/A'}</p>
                                                </div>
                                                <div>
                                                    <p className="text-xs text-gray-500 mb-0.5">Username</p>
                                                    <p className="text-xs sm:text-sm font-medium text-gray-900 truncate">@{account.username || 'N/A'}</p>
                                                </div>
                                                <div>
                                                    <p className="text-xs text-gray-500 mb-0.5">Email</p>
                                                    <p className="text-xs sm:text-sm font-medium text-gray-900 truncate">{account.email || 'N/A'}</p>
                                                </div>
                                                <div>
                                                    <p className="text-xs text-gray-500 mb-0.5">Phone</p>
                                                    <p className="text-xs sm:text-sm font-medium text-gray-900 truncate">{account.phone || 'N/A'}</p>
                                                </div>
                                                <div>
                                                    <p className="text-xs text-gray-500 mb-0.5">Role</p>
                                                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${account.role === 'admin' ? 'bg-gradient-to-r from-blue-400 to-cyan-500 text-white' :
                                                        account.role === 'manager' ? 'bg-gradient-to-r from-green-400 to-emerald-500 text-white' :
                                                ...
                                                <div>
                                                    ...
                                                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${account.acc_status === 'active' ? 'bg-gradient-to-r from-green-400 to-emerald-500 text-white' :
                                                        account.acc_status === 'suspended' ? 'bg-gradient-to-r from-yellow-400 to-orange-500 text-white' :
                                                            account.acc_status === 'inactive' ? 'bg-red-600 text-white' :
                                                                'bg-gray-100 text-gray-800'
                                                        } capitalize`}>
                                ...
                                <div className="bg-gray-50 rounded-lg border p-2.5 sm:p-3" style={{ borderColor: '#A86523' }}>
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 sm:gap-2">
                                        <div>
                                            ...
                                            <p className="text-xs sm:text-sm font-medium text-gray-900">{account.gender || 'N/A'}</p>
                                        </div>
                                        <div>
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs sm:text-sm font-medium text-gray-900 break-words">{account.address || 'N/A'}</p>
                                        </div>
                                        <div>
                                ...
                                {account.role !== 'admin' && internalViewOnly && (
                                    <div className="bg-gray-50 rounded-lg border p-2.5 sm:p-3" style={{ borderColor: '#A86523' }}>
